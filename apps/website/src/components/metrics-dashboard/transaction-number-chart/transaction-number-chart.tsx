...
export const TransactionNumberChart = () => {
  ...
  const transactionNumberChartData = folksRouterData.map(({ periodStartUnix, routeTransactionCount }) => ({
    "transaction-number": new Date(periodStartUnix).toLocaleDateString("en-US", {
      hour: "2-digit",
      day: "2-digit",
      month: "short",
      year: "numeric",
  ...
  return (
    <Card className="flex flex-col gap-y-12">
      ...
      <BarChart
        data={transactionNumberChartData}
        index="transaction-number"
        categories={["Transaction Number"]}
        colors={["teal"]}
        yAxisWidth={80}
        showAnimation
        showXAxis={false}
